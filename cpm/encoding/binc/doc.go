...
The encoding/binc package encodes and decodes binc data.
	...
	var lib = require('encoding/binc');
	var enc = lib.encode({ test: true });
	var dec = lib.decode(enc);
	console.log(enc, dec);

It is also possible to decode binc data directly from a reader.
...
It is also possible to encode binc data directly to a writer.
