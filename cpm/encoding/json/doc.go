...
The encoding/json package encodes and decodes json data.
	...
	var lib = require('encoding/json');
	var enc = lib.encode({ test: true });
	var dec = lib.decode(enc);
	console.log(enc, dec);

It is also possible to decode json data directly from a reader.
...
It is also possible to encode json data directly to a writer.
