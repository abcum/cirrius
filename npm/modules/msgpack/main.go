...
func init() {
	...
	orbit.Add("msgpack", func(ctx *orbit.Orbit) (otto.Value, error) {
		return ctx.ToValue(map[string]interface{}{
			"encode": func(call otto.FunctionCall) (val otto.Value) {

				if len(call.ArgumentList) > 1 {
					msg := fmt.Sprintf("Too many arguments to 'msgpack.encode'. Expected 1 argument, but received %d.", len(call.ArgumentList))
